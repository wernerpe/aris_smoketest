...
| Lateral tip offset | ~~0.110~~ → ~~0.0588421~~ → **0.1253421 m along hand x** (perpendicular to finger travel): tip = TCP + R @ (0.0588421, 0, 0.0588421) | **RE-SPECIFIED 2026-09-03, then again 2026-09-04.**  The AXIAL half comes from the tip sitting ~5 cm below the blades' contact plates (z 0.1122421); the LATERAL half is now `grip_hand_x + PEN_EXT_HOLDER · tan 45°` = 0.066500 + 0.0588421, because the grip moved to the plates' far end.  USER-SPECIFIED, **never gate-validated** — that wording was wrong wherever it appeared |
...
| Lean out of the approach axis | **45°**, `frames.PEN_LEAN_HOLDER` — the **BORE's** lean, measured at the GRIP.  The TCP→tip ray is 64.85° and is a different thing now | **CHOSEN, not forced — the "forced" claim of 2026-09-03 is WITHDRAWN (2026-09-04).**  On the finger centreline the casing ruled out any lean under 35.17°; at the far-end placement the photo shows, 23° clears the casing by **13.84 mm** against 45°'s **7.78**, so the hardware constrains nothing.  45° is the user's standing rule and the incumbent.  Per the user, casing intersection does not constrain the model at all while the pencil's modelled length is arbitrary.  `docs/SYSTEM_MODEL.md` §7e |
