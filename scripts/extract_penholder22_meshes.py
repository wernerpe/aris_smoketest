...
def outside_envelope(pts):
    """How far each hand-frame point lies OUTSIDE the collision envelope.
    ...
    for kind, T, par in rig_final.penholder22_collision(
...
def main():
    ...
    print(f"PLANNER asks for a {lean:.2f} deg lean and {reach * 1000:.2f} mm "
          ...
          f"deg and {nose * 1000:.2f} mm grip-to-nose, so the drawing needs "
          f"{(reach - nose) * 1000:.1f} mm of graphite past the nose and "
