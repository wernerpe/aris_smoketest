...
- **LATERAL HOLDER (2026-08-25, env-selectable, NOT default; RE-SPECIFIED
  ...
  rather than only the lateral one. The tip is derived in one line — grip at
  (0.0665, 0, 0.1034), the plates' FAR END; bore at the housing's own **23°**,
  which is the only lean that puts its square block flush with the blades;
  20 mm of graphite past the cap — giving `PEN_EXT_HOLDER` **0.0460262** and
  `PEN_LAT_HOLDER` **0.0860369**. They were 0.110 / 0.110 and were **never
  gate-validated**; only the inline pen's axial 0.110 is
...
tip moved three times on the photo of the real gripper — it is
`TCP + R @ (0.0860369, 0, 0.0460262)` now, the holder clamped at the far end
of the Fat finger plates with its bore at the housing's own 23° — and **the
CAD's last disagreement is gone**: the housing's "22 deg" is a clocking about
the mount post, it measures **23.00°**, and that is now also the lean the
model draws, because the square block is flush with the blades at 23° and
nowhere else.
...
instead — to `TCP + R @ (0.0860369, 0, 0.0460262)`, the grip at the far end
of the blades and the bore at the housing's own 23 deg — and it was **never**
...
on the finger centreline and did not survive the move; 2026-09-07 then put
the lean back where the CAD always had it (§7e).  The same
