...
def test_phasing_by_arm_conducts_the_same_ink_one_group_at_a_time(tmp_path):
    ...
    # THE GATE THAT USED TO NEED AN OPT-IN.  Once the mesh audit widened the
    ...
    # this repo does not allow.  This test bought the relationship back with
    # `--frame-safe`; since 2026-08-26 `paper.STATIC_SAFE` is ON and every
    # pen-up is certified against the steel and the neighbours' base columns
    # without being asked, so the flag is gone and the phase runs by default.
    a = ap.parse_args(["--arms", "13,31", "--no-prefilter", "--no-balance",
